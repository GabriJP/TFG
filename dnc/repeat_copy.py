...
class RepeatCopy(snt.AbstractModule):
    ...
    def _build(self):
        ...
        residual_obs_pad = [
            tf.zeros([tf.gather(residual_length_batch, i), full_obs_size])
            for i in range(batch_size)
        ]
        residual_targ_pad = [
            tf.zeros([tf.gather(residual_length_batch, i), full_targ_size])
            for i in range(batch_size)
        ]
        residual_mask_pad = [
            tf.zeros([tf.gather(residual_length_batch, i)]) for i in range(batch_size)
        ]
    ...
    def to_human_readable(self, data, model_output=None, whole_batch=False):
        ...
        unnormalised_num_reps_flag = self._unnormalise(obs[:, :, -1:])
