...
with open('deep.txt', 'w') as csv_file:
    csv_writer = csv.writer(csv_file)
    for data_dir in ("data/tasks_1-20_v1-2/en-10k", "../familytask"):
        for task_id in (3,):
            for hops in range(6, 10):
                ...
                with tf.Session() as sess:
                    csv_writer.writerows((
                        ("hops", "task_id", "data_dir", "Training Size", "Validation Size", "Testing Size"),
                        (hops, task_id, data_dir, n_train, n_val, n_test),
                        (),
                        ("Epoch", "Total cost", "Training accuracy", "Validation Accuracy")
                    ))
                    csv_file.flush()
                    model = MemN2N(batch_size, vocab_size, sentence_size, memory_size, FLAGS.embedding_size, session=sess,
                                   hops=hops, max_grad_norm=FLAGS.max_grad_norm)
                    for t in range(1, FLAGS.epochs + 1):
                        ...
                        if t % FLAGS.evaluation_interval == 0:
                            ...
                            print('-----------------------')
                            csv_writer.writerow((t, total_cost, train_acc, val_acc))
                            csv_file.flush()
